...
### v11 Engineering Iteration (2026-08-05)

An engineering iteration addressing the Phase 2 verification run's limitations (plum-only dataset) produced a multi-fruit dataset and retrained SmolVLA:

| Metric | Phase 2 Baseline | v11 Iteration |
|--------|------------------|---------------|
| Dataset | 5 episodes (plum only) | 24 episodes (13 plum, 7 banana, 4 lemon) |
| Training steps | 2000 | 16000 |
| Final loss | 0.064 | 0.021 |
| **Overall success rate** | **13.9% (5/36)** | **25.0% (9/36)** |
| Plum success | 41.7% (5/12) | **66.7% (8/12)** |
| Banana success | 0.0% (0/12) | 8.3% (1/12) |
| Lemon success | 0.0% (0/12) | 0.0% (0/12) |

**Key engineering fixes in v11:**
1. **`GRASP_CENTER_DROP_FRAC` bug fix** — Record-time override (1.0) caused lemon/plum fingertips to embed in fruit bottom, flinging them. Restored to 0.45 with per-profile overrides.
...
3. **IK branch flip fix** — `_descend_vertical` used different IK branches before/after `_go_home`, causing 2cm hand drift. Unified to `_goto_direct` for all fruits.

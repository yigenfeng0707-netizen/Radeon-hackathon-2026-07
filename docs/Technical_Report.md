...
### 7.1.1 v11 Engineering Iteration — Multi-Fruit Dataset

Following the Phase 2 verification run, an engineering iteration addressed the root cause (plum-only dataset) by fixing five bugs in the scripted policy and recording a multi-fruit dataset:

**Bug fixes applied:**
1. **`GRASP_CENTER_DROP_FRAC` override** — Record-time patch set this to 1.0, causing fingertips to embed in fruit bottoms. Restored to 0.45 with per-profile overrides.
2. **Per-fruit grasp profiles** — Lemon/plum: -12N close force (was -60N/-80N), center_align=True, center_drop_frac=0.45. Banana: -50N, yaw_offset=-35°.
3. **IK branch flip** — `_descend_vertical` used different IK branches before/after `_go_home`, causing 2cm hand drift. Unified to `_goto_direct`.
4. **Camera coordinate system** — `camera1/camera2` → `world/wrist` renaming for LeRobot compatibility.
5. **SmolVLA `torch_compilable_check`** — Workaround for missing symbol in transformers 4.57.6.

**v11 dataset:** 24 episodes (13 plum, 7 banana, 4 lemon), 8321 frames, merged from three recording sessions.

**v11 training:** 16000 steps, final loss 0.021, ~45 minutes on W7900.

**v11 evaluation (12×3 robustness matrix):**

| Fruit | Successes / Trials | Success Rate | Mean Steps on Success |
|---|---:|---:|---:|
| `018_plum` | 8 / 12 | 66.7% | 370.9 |
| `011_banana` | 1 / 12 | 8.3% | 341.0 |
| `014_lemon` | 0 / 12 | 0.0% | N/A |
| **Overall** | **9 / 36** | **25.0%** | — |

This represents a **+11.1pp improvement** over the Phase 2 verification run (13.9%), with plum success nearly doubling (41.7% → 66.7%). The canonical headline remains the baseline 2/2 = 100% in `logs/eval_results.json`.
